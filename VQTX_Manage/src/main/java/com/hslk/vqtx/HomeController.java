...
public class HomeController {
	...
	public void addScore(HttpServletRequest request, HttpServletResponse response) {
		...
		if (teamCode != "0") {
			try {
				scoreService.addScore(stationCode, teamCode, score1, score2, score3, bonus, bonusNote, penalty,
						penaltyNote, note, user.getUsername());
				try {
					response.sendRedirect(chiefPage);
				} catch (IOException ex) {
					ex.printStackTrace();
				...
				try {
					response.sendError(HttpServletResponse.SC_BAD_REQUEST,
							"Đã có lỗi khi nhập điểm, hãy thử lại lần nữa." + "\n Chú ý quy tắc nhập điểm");
				} catch (IOException ex1) {
					LOGGER.error("Send redirect error occur exception!");
				}
			}
